...
static const uint64_t INPUT_END   = 65536UL * 256UL;
...
int is_correct(thread_context_t* threadContext, uint64_t input, uint64_t actual_result, uint64_t* expected) {
    uint64_t a = input & 65535UL;
    uint64_t b = input / 65536UL;

    uint64_t e = (a*b) / 256UL;
    *expected = e;
